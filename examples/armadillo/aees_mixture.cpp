...
// -I$ARMA_INCLUDE_PATH -I./../../include/ aees_mixture.cpp -o aees_mixture.out
// -L./../.. -lmcmc
...
double gaussian_mixture(const arma::vec &X_vec_inp, const arma::vec &weights,
                        const arma::mat &mu, const arma::vec &sig_sq) {
...
double target_log_kernel(const arma::vec &vals_inp, void *target_data) {
...
int main() {
  ...
  arma::cout << "posterior mean for > 0.1:\n"
             << arma::mean(draws_out.elem(arma::find(draws_out > 0.1)), 0)
             << arma::endl;
  arma::cout << "posterior mean for < -0.1:\n"
             << arma::mean(draws_out.elem(arma::find(draws_out < -0.1)), 0)
             << arma::endl;
