...
public:
  ...
  explicit Cube_t();
  explicit Cube_t(const size_t n_row_inp, const size_t n_col_inp,
...
template <typename T>
inline Cube_t<T>::Cube_t() : n_row(0), n_col(0), n_mat(0) {
  raw_data_ = Cube_t<T>::CubeData_t(0, Cube_t<T>::MatData_t(0, 0));
}

template <typename T>
inline Cube_t<T>::Cube_t(const size_t n_row_inp, const size_t n_col_inp,
                         const size_t n_mat_inp)
    : n_row(n_row_inp), n_col(n_col_inp), n_mat(n_mat_inp) {
  raw_data_ = Cube_t<T>::CubeData_t(n_mat_inp,
                                    Cube_t<T>::MatData_t(n_row_inp, n_col_inp));
}

template <typename T>
inline Cube_t<T>::Cube_t(const Cube_t<T> &cube_inp)
    : n_row(cube_inp.n_row), n_col(cube_inp.n_col), n_mat(cube_inp.n_mat) {
  raw_data_ = cube_inp.get_raw_data();
}

template <typename T>
inline Cube_t<T>::Cube_t(Cube_t<T> &&cube_inp)
    : n_row(cube_inp.n_row), n_col(cube_inp.n_col), n_mat(cube_inp.n_mat) {
...
template <typename T>
inline Cube_t<T> &Cube_t<T>::operator=(const Cube_t<T> &cube_inp) {
  raw_data_ = cube_inp.get_raw_data();
...
template <typename T>
inline Cube_t<T> &Cube_t<T>::operator=(Cube_t<T> &&cube_inp) {
  raw_data_ = std::move(cube_inp.get_raw_data());
...
template <typename T>
inline T &Cube_t<T>::operator()(const size_t row_ind, const size_t col_ind,
                                const size_t mat_ind) {
  return raw_data_[mat_ind](row_ind, col_ind);
}

template <typename T>
inline const T &Cube_t<T>::operator()(const size_t row_ind,
...
template <typename T>
inline typename Cube_t<T>::MatData_t &Cube_t<T>::mat(const size_t mat_ind) {
  return raw_data_[mat_ind];
}

template <typename T>
inline const typename Cube_t<T>::MatData_t &
Cube_t<T>::mat(const size_t mat_ind) const {
  return raw_data_[mat_ind];
}

// set

template <typename T> inline void Cube_t<T>::setZero() {
  if (n_mat > size_t(0)) {
...
template <typename T>
inline void Cube_t<T>::setZero(const size_t n_row_inp, const size_t n_col_inp,
                               const size_t n_mat_inp) {
...
template <typename T>
inline typename Cube_t<T>::CubeData_t &Cube_t<T>::get_raw_data() {
  return raw_data_;
}

template <typename T>
inline const typename Cube_t<T>::CubeData_t &Cube_t<T>::get_raw_data() const {
  return raw_data_;
}

template <typename T> inline void Cube_t<T>::reset_raw_data() {
  raw_data_.clear();
}

template <typename T> inline void Cube_t<T>::reset_dims() {
...
template <typename T>
inline void Cube_t<T>::set_dims(const size_t n_row_inp, const size_t n_col_inp,
                                const size_t n_mat_inp) {
