...
// $CXX -Wall -std=c++17 -O3 -mcpu=native -ffp-contract=fast
// -I$EIGEN_INCLUDE_PATH -I$AUTODIFF_INCLUDE_PATH  -I./../../include/
// hmc_normal_autodiff.cpp -o hmc_normal_autodiff.out -L./../.. -lmcmc
...
double ll_dens(const Eigen::VectorXd &vals_inp, Eigen::VectorXd *grad_out,
  ...
  std::function<autodiff::real(const autodiff::ArrayXreal &vals_inp)>
      normal_dens_log_form =
          [x, pi](const autodiff::ArrayXreal &vals_inp) -> autodiff::real {
    autodiff::real mu = vals_inp(0);
    autodiff::real sigma = vals_inp(1);

    return -x.size() * (0.5 * std::log(2 * pi) + autodiff::detail::log(sigma)) -
  ...
  if (grad_out) {
    Eigen::VectorXd grad_tmp = autodiff::gradient(
        normal_dens_log_form, autodiff::wrt(xd), autodiff::at(xd), u);
